...
window.enviarFotos = async function (consultaId, pacienteId, medicoId, grupoEvolucao = 'geral') {
    ...
    for (let idx = 0; idx < window.fotosConsulta.length; idx++) {
        ...
        try {
            ...
            // Insere direto no Supabase via SDK (Garante que a imagem aparece na aba de imagens)
            let dbData = null;
            if (window.supabaseClient) {
                ...
                if (!error) {
                    dbData = data;
                    enviadas.push(data);
                    console.log(`[app-images] ✅ Foto ${idx + 1} registrada no BD via SDK`);
                } else {
                    console.warn(`[app-images] Insert BD falhou:`, error);
                    erros.push({ idx, err: error.message });
                }
            }

            // Opcional: notifica o webhook n8n se precisar de automação extra
            try {
                fetch(`${IMG_WEBHOOK}/uploadFotosConsulta`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload),
                }).catch(e => console.warn('Webhook silencioso falhou:', e));
            } catch (webhookErr) {
                // ignorar
            }
        } catch (err) {
