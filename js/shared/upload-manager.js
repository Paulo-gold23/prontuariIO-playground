/**
 * js/shared/upload-manager.js
 * ─────────────────────────────────────────────────────────────
 * Módulo de Upload de Imagens Compartilhado (Multi-tenant)
 * Responsabilidade: upload, listagem e exclusão de imagens no bucket 'anexos_imagens'
 * Parametrizado para suportar diferentes clínicas através de clinica_id/grupo_evolucao.
...
const UploadManager = (() => {
   ...
   * @param {string} grupoEvolucao — opcional (se undefined, detecta do medicoAtivo ou usa 'cicatrize')
   * @returns {Promise<{success: boolean, data?: object, error?: string}>}
   */
  async function uploadArquivo(arquivo, pacienteId, medicoId, consultaId = null, descricao = '', grupoEvolucao = undefined) {
    // Validação de tipo
    ...
    const grupoEvolucaoFinal = grupoEvolucao !== undefined ? grupoEvolucao : (medicoLocal.tipo_clinica || 'cicatrize');
    ...
    const { data: dbData, error: dbErr } = await supabase
      ...
      .insert({
        ...
        grupo_evolucao: grupoEvolucaoFinal
      })
      .select()
      .single();
  ...
  async function processarUpload(arquivo, pacienteId, medicoId, grid, statusEl) {
    ...
    const result = await uploadArquivo(arquivo, pacienteId, medicoIdFinal, null, '', medicoLocal.tipo_clinica);
